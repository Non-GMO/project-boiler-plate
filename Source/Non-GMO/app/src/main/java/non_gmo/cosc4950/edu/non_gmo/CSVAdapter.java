...
public class CSVAdapter extends ArrayAdapter<Category> {
    Context ctx;
    ...
    public View getView(final int pos, View convertView, final ViewGroup parent){
        ...
        mView.setText(getItem(pos).getCategory());

        return mView;
    }

    /*
         * Helper method that loads the data from the ngp.csv and builds
         * each csv row into a Category object which then gets added to the Adapter.
         */
    private void loadArrayFromFile() {
        try {
            // Get input stream and Buffered Reader for our data file.
            InputStream is = ctx.getAssets().open("ngp.csv");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                //Split to separate the category from everything else????????????
                String[] RowData = line.split(",");

                if(RowData.length <2)
                    continue;

                //Create a Category object for this row's data.
                Category cur = new Category();
                cur.setCategory(RowData[0]);

                //Add the Category object to the ArrayList (in this case we are the ArrayList).
                this.add(cur);
            }
        } catch (IOException e) {
            e.printStackTrace();
